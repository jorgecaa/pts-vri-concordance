#!/usr/bin/env python3
"""reconcile_sn1.py — vuelca `validador_sn1.json` al Excel maestro (SN I, S i).
...
A diferencia de SN V, en SN I la alineación es **posicional** y viene respaldada por tres
comprobaciones independientes (estructura de Feer, secuencia canónica, y `cst_p_page` para los
271), así que un REJECT es un problema de *cotejo de texto* (ortografía de Feer, elisión), nunca
de alineación.

Uso:
    python3 reconcile_sn1.py --dry
    python3 reconcile_sn1.py --write [--humano firmas.json]
"""
...
RES = 'validador_sn1.json'


def sn1_rows(ws, ci):
    for ridx, row in enumerate(ws.iter_rows(min_row=2), start=2):
        v = [c.value for c in row]
        if str(v[ci['Nikaya']] or '') != 'SN' or str(v[ci['PTS Roman']] or '').strip().lower() != 'i':
            continue
...
def main():
    ...
    for ridx, num, v in sn1_rows(ws, ci):
        if num in humano:
            val = humano[num]
    ...
    if dry:
        ...
        return
    bak = f'{XLSX}.backup-{datetime.datetime.now():%Y%m%d-%H%M%S}-sn1.xlsx'
