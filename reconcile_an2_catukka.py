#!/usr/bin/env python3
"""reconcile_an2_catukka.py — vuelca `validador_an2_catukka.json` al Excel maestro (AN II).
...
Escribe además la **`VRI Ref`** (`s0402m3:N`), la clave canónica del lado CST (regla (5)).
...
⚠️ Las **28 filas de rango** de A ii no entran aquí: son redundantes con filas individuales que ya
existen y están a arbitraje, como `12.74` (S ii) y `22.148` (S iii).

Uso:
    python3 reconcile_an2_catukka.py --dry
    python3 reconcile_an2_catukka.py --write [--humano firmas.json]
"""
import datetime
import json
import shutil
...
RES = 'validador_an2_catukka.json'
...
def main():
    ...
    for row in ws.iter_rows(min_row=2):
        ...
        if str(v[ci['Nikaya']] or '') != 'AN' or str(v[ci['PTS Roman']] or '').strip().lower() != 'ii':
            continue
        num = str(v[ci['Sutta #']])
    ...
    if dry:
        ...
        return
    bak = f'{XLSX}.backup-{datetime.datetime.now():%Y%m%d-%H%M%S}-an2catukka.xlsx'
