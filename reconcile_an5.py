#!/usr/bin/env python3
"""reconcile_an5.py — vuelca `validador_an5.json` al Excel maestro (AN V: Dasaka y Ekādasaka).
...
Escribe además la **`VRI Ref`**, la clave canónica del lado CST (regla (5)); el fichero depende de
la sección (`s0404m3` Dasaka, `s0404m4` Ekādasaka) y lo trae cada registro del JSON.
...
⚠️ Quedan **fuera** las filas de rango y las **16 sin alinear** (cola de peyyāla, donde el CST no
nombra por sutta). Ninguna se toca.

Uso:
    python3 reconcile_an5.py --dry
    python3 reconcile_an5.py --write [--humano firmas.json]
"""
import datetime
import json
import shutil
...
def main():
    ...
    for row in ws.iter_rows(min_row=2):
        ...
        if str(v[ci['Nikaya']] or '') != 'AN' or str(v[ci['PTS Roman']] or '').strip().lower() != 'v':
            continue
        num = str(v[ci['Sutta #']])
        if not any(num.startswith(p) for p in ('10.', '11.')):
            continue
        if '-' in num:
    ...
    if dry:
        ...
        return
    bak = f'{XLSX}.backup-{datetime.datetime.now():%Y%m%d-%H%M%S}-an5.xlsx'
