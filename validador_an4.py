#!/usr/bin/env python3
"""Aligner + validador de **AN IV** (A iv, `book_no=20`): Sattaka, Aṭṭhaka y Navaka nipāta.

Mismo régimen que A iii —numeral **romano centrado** como nº de sutta, arábigos a sangría 5 como
párrafos— y misma receta. Tres secciones en vez de dos.

**Verdad-terreno del lado PTS** (`anguttara-vol-IV-info.txt`, índice de Hardy 1899): 27 vaggas con
numeración **corrida a través de los tres nipātas** (1-9 Sattaka, 10-18 Aṭṭhaka, 19-27 Navaka), no
reiniciada por nipāta como en A i.

Marcadores: **Sattaka 80, Aṭṭhaka 90, Navaka 100**, sin un solo hueco — tras reparar cuatro erratas
del romano en el impreso/OCR (`LI. *)`, `LX XXVII.` con un espacio dentro del numeral, `I 2.` y el
rango `LXXIII-LXXXI6.`; ver `an1_markers._limpia_romano`).

Colofones: 8/9, 9/9, 9/9. El único ausente es el `[Vaggasaṅgahitā]` del Sattaka, que **Hardy titula
entre corchetes por ser conjetura suya** y no tiene colofón en los MSS — coherencia, no defecto.

⚠️ **Divergencia de recuento que Hardy declara él mismo** (Preliminary Remarks):
    «In the Sattaka-Nipāta, Vagga II the Commentary counts No. XVII as sattame, aṭṭhame, navame…
     we should therefore substitute Nos. XVII—XIX for our No. XVII, and accordingly XX for XVIII…»
Es decir: **un sutta de PTS = tres del Comentario** en ese punto, y desde ahí la numeración corre
desplazada hasta reencontrarse. **No es un offset constante**; por eso el emparejamiento va por
alineamiento monótono y nunca por clave.

⚠️ **Los ficheros VRI no se reparten como los tomos de PTS**: el Sattaka está en `s0403m3` —el
mismo fichero cuyos hermanos son de A iii— y el Aṭṭhaka y el Navaka en `s0404m1` y `s0404m2`.

Uso: python3 validador_an4.py [--dry] [--all] [--n N] [--only 7.5,8.2]
"""
import json
import os
import re
...
SECCIONES = [('SATTAKA', 's0403m3', '7.'), ('AṬṬHAKA', 's0404m1', '8.'),
             ('NAVAKA', 's0404m2', '9.')]
OUT = 'validador_an4.json'
...
HARDY = {
    'SATTAKA': [(1, 'Dhana', 1), (2, 'Anusaya', 9), (3, 'Vajji', 16), (4, 'Devatā', 27),
                (5, 'Mahāyañña', 39), (6, 'Avyākata', 67), (7, 'Mahā', 99), (8, 'Vinaya', 140),
                (9, 'Vaggasaṅgahitā', 144)],
    'AṬṬHAKA': [(1, 'Mettā', 150), (2, 'Mahā', 172), (3, 'Gahapati', 208), (4, 'Dāna', 236),
                (5, 'Uposatha', 248), (6, 'Sa-ādhāna', 274), (7, 'Bhūmicāla', 293),
                (8, 'Yamaka', 314), (9, 'Sati', 336)],
    'NAVAKA': [(1, 'Sambodha', 351), (2, 'Sīhanāda', 373), (3, 'Sattāvāsa', 396),
               (4, 'Mahā', 410), (5, 'Pañcāla', 449), (6, 'Khema', 455),
               (7, 'Satipaṭṭhāna', 457), (8, 'Sammappadhāna', 462), (9, 'Iddhipāda', 463)],
}
...
def excel_entries(pref):
    ...
    for row in ws.iter_rows(min_row=2, values_only=True):
        if str(row[ci['Nikaya']] or '') != 'AN' or str(row[ci['PTS Roman']] or '').strip().lower() != 'iv':
            continue
...
def colofones(cur, nipata):
    ...
    pages = {r['page_no']: (r['unitext'] or '').split('\n')
             for r in cur.execute('SELECT page_no,unitext FROM pages '
                                  'WHERE edition="mula" AND book_no=?', (AN4_BOOK,))}
...
def main():
    ...
    print('=' * 92)
    print('AN IV — SATTAKA, AṬṬHAKA y NAVAKA: pipeline VRI (anguttara-vol-IV-info.txt)')
    print('=' * 92)
