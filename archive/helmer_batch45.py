#!/usr/bin/env python3
"""SN IV Helmer — remaining batches (suttas 31+)."""
...
# Get exact-match tasks for entries 30+
tasks=[]
for e in entries[30:]:
...
print('Batches 4-5: {} with exact match'.format(len(tasks)))
...
if acc>0:
    print('\nBatches 4-5: APPROVE {} REJECT {} = {:.0f}%'.format(vc.get('APPROVE',0),vc.get('REJECT',0),100*vc.get('APPROVE',0)/acc))
conn.close()
