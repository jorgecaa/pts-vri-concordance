#!/usr/bin/env python3
"""Helmer Smith — PTS↔CST cross-validation. DN+MN (1:1 CST mapping)."""
...
DN_BOOKS={'i':6,'ii':7,'iii':8}; MN_BOOKS={'i':9,'ii':10,'iii':11}

def cst_for_dnm(nik, snum):
    sn=int(snum)
    if nik=='DN':
...
def pts_text(book_no, page_no, line_no=1, n_tokens=350):
...
SYS="""You are Helmer Smith, Pāli philologist. You audit whether two Pāli passages (PTS and CST editions) are the SAME sutta. Same=core narrative matches; editions may differ in orthography, elisions (peyyāla), or one may have expanded passages. Different=completely distinct sutta. Reply ONLY JSON: {"verdict":"APPROVE"|"REJECT","reason":"<=15 words English"}"""

def ask(p,c,v,cli):
    body=['PTS:',p[:1200],'','CST:',c[:1200],'','CollateX (PTS|CST|DPD):']
    for sa,sb,tag in v: body.append(f'  "{sa}" | "{sb}" | {tag}')
...
def select(n=30):
    ...
    for ri in range(2,ws.max_row+1):
        ...
        if nik=='DN': bn=DN_BOOKS.get(roman)
        else: bn=MN_BOOKS.get(roman)
        if not bn or not page: continue
        tests.append({'nik':nik,'num':snum,'ref':ref,'name':name,'bn':bn,'page':page,'line':line,'val':val})
    import random; random.seed(42); random.shuffle(tests)
    return tests[:n]
...
def main():
    ...
    print(f'Preloading CST...')
    cst_cache={}
    for t in tests:
        xf,lv,idx=cst_for_dnm(t['nik'],t['num']); k=tuple(xf)
        if k not in cst_cache: cst_cache[k]=sh.load_cha(list(xf),lv)
    
    print(f'Running {n} PTS↔CST validations (DN+MN, 350 tokens)...')
    print('='*95)
    results=[]
    
    for t in tests:
        xf,lv,cst_idx=cst_for_dnm(t['nik'],t['num'])
        if xf is None:
            results.append((t,{'verdict':'ERROR','reason':'no CST mapping','nvar':0},'')); continue
        
        pts=pts_text(t['bn'],t['page'],t['line'])
        if not pts:
            results.append((t,{'verdict':'ERROR','reason':'PTS page missing','nvar':0},'')); continue
        
        segs=cst_cache[tuple(xf)]
        if cst_idx>=len(segs):
            results.append((t,{'verdict':'ERROR','reason':f'CST idx {cst_idx} >= {len(segs)}','nvar':0},'')); continue
        
        cst_sutta=segs[cst_idx]; cst_text=' '.join(sh.tokens(cst_sutta['text'])[:350])
        ...
        if key in cache: hv=cache[key]
            ...
            except Exception as e: hv={'verdict':'ERROR','reason':str(e)[:60],'nvar':0}
            cache[key]=hv
            CACHE_FILE.write_text(json.dumps(cache,ensure_ascii=False,indent=2))
        
        results.append((t,hv,cst_title))
        v=hv.get('verdict','?'); sym='✓' if v=='APPROVE' else '✗' if v=='REJECT' else '?'
        print(f'{sym} {t["nik"]} {t["num"]:>6s} {t["ref"]:>18s} | {v:8s} | CST: {cst_title[:40]} | {hv.get("reason","")[:55]}')
    
    vc=Counter(hv.get('verdict','?') for _,hv,_ in results)
    total=len(results); app=vc.get('APPROVE',0); rej=vc.get('REJECT',0); err=vc.get('ERROR',0)
    print(f'\n{"="*95}')
    print(f'  APPROVE (Excel ✓): {app} | REJECT: {rej} | ERROR: {err}')
    valid=total-err
    if valid>0: print(f'  Precisión Excel: {app}/{valid} ({100*app/valid:.0f}%)')
    if rej>0:
        print(f'\n  ⚠ Revisar:')
        for t,hv,cst_title in results:
            if hv.get('verdict')=='REJECT':
                print(f'    {t["nik"]} {t["num"]:>6s} | {t["ref"]} | CST: {cst_title[:30]} | {hv.get("reason","")[:60]}')
